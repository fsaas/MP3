...
BlockManager::BlockManager()
{
	...
	block[88] = new Block("탄환 생성", "탄막의 각도, 속도, 개수 모두 랜덤 값으로 플레이어 위치에 생성된다.");
	block[89] = new Block("탄환 이동", "설정된 옵션대로 탄환이 개별적으로 이동한다.");
	block[90] = new Block("탄환 재생성", "탄환의 각도, 속도, 개수 모두 랜덤으로 재생성한다.");
	
	block[91] = new Block("탄환 재생성", "탄환의 각도 속도, 개수의 값을 늘린 후에 재생성한다.");
	block[92] = new Block("화면 진동", "탄이 발사될 때 화면이 흔들리도록 한다.");
	block[93] = new Block("탄환 속도 변경", "발사된 탄환은 속도가 점점 빨라진다.");
	block[94] = new Block("탄환 방향 바꾸기", "발사된 탄환은 이동 중에 랜덤으로 방향을 바꾼다.");
	

	// #7 서로 다른 패턴을 가진 탄을 동시에 발사하기

	block[95] = new Block("패턴1 캐릭터 생성", "개발자가 지정한 패턴1의 탄막을 생성하는 캐릭터를 생성한다");
	block[96] = new Block("패턴2 캐릭터 생성", "개발자가 지정한 패턴2의 탄막을 생성하는 캐릭터를 생성한다");
	block[97] = new Block("패턴1과 패턴2의 캐릭터를 합치기", "패턴1의 캐릭터와 패턴2의 캐릭터를 같은 위치에 두고 한 캐릭터는 이미지를 생성하지 않는다.");
	block[98] = new Block("탄환 발사", "패턴1의 캐릭터와 패턴2의 캐릭터가 동시에 탄환을 생성한다.");

	block[99] = new Block("탄막제거", "화면을 넘어가는 탄막은 제거한다");
	block[100] = new Block("패턴1 캐릭터 2개를 합치기", "패턴1의 캐릭터를 2개 생성하여 같은 위치에 생성한다.");
	block[101] = new Block("패턴2 캐릭터 2개를 합치기", "패턴2의 캐릭터를 2개 생성하여 같은 위치에 생성한다.");
	block[102] = new Block("탄환 발사", "패턴1의 캐릭터가 탄환을 생성하고 나서 패턴2의 캐릭터가 탄환을 생성한다.");


	// #8 유도탄
	
	block[103] = new Block("최대 각도, 속도 설정", "캐릭터의 탄막의 최대 각도와 최대 속도를 설정한 채 생성한다.");
	block[104] = new Block("플레이어 위치 확인", "발사된 탄막은 플레이어의 위치를 지속적으로 받는다.");
	block[105] = new Block("탄막 이동", "탄막이 플레이어가 있는 방향으로 이동한다.");
	block[106] = new Block("탄막의 이동 제한", "받는 플레이어의 위치로 이동하지만, 설정된 최대 각도, 속도보다 크거나 빠르지 않게 이동한다.");

	block[107] = new Block("탄막 이동", "탄막이 플레이어가 있는 위치로 이동한다.");
	block[108] = new Block("충돌", "플레이어와 탄막이 충돌범위가 겹친다.");
	block[109] = new Block("탄막 속도 감소", "플레이어와 가까워지면 탄환의 속도를 감소시킨다.");
	block[110] = new Block("탄막 삭제", "화면을 넘어가면 탄막을 삭제한다.");


	// #9 충돌판정 - 탄막
	
	block[111] = new Block("충돌범위 생성", "탄막과 캐릭터는 개개인의 충돌범위를 가진다.");
	block[112] = new Block("충돌", "탄막과 캐릭터의 충돌 범위가 겹친다.");
	block[113] = new Block("HP 감소", "충돌한 캐릭터의 HP는 감소한다.");
	block[114] = new Block("탄환 삭제", "충돌한 탄환은 삭제된다.");

	block[115] = new Block("탄막 삭제", "화면을 넘어간 탄환은 삭제한다.");
	block[116] = new Block("캐릭터 폭발", "HP가 0이된 캐릭터는 광범위한 피해를 입힌다.");
	block[117] = new Block("탄막 충돌", "각 캐릭터가 발사한 탄환끼리 충돌할 경우 두 탄환을 삭제한다.");
	block[118] = new Block("캐릭터 삭제", "현재 화면상에 존재하는 캐릭터를 삭제한다.");


	// #10 충돌판정 - 적기와 플레이어

	block[119] = new Block("충돌범위 생성", "각 캐릭터는 개개별로 충돌범위를 가진다.");
	block[120] = new Block("충돌", "서로 적대하는 캐릭터간의 충돌범위가 겹친다.");
	block[121] = new Block("HP 감소", "충돌한 캐릭터는 개개인의 HP가 감소한다.");
	block[122] = new Block("캐릭터 HP 확인", "HP가 0이 된 캐릭터는 삭제하며, 플레이어의 캐릭터가 HP 0이되면 게임오버된다.");
	
	block[123] = new Block("게임오버", "캐릭터의 HP가 0이되었을 경우 게임을 종료시킨다.");
	block[124] = new Block("아이템 초기화", "캐릭터가 가지고 있던 아이템을 초기화한다.");
	block[125] = new Block("캐릭터 이동", "캐릭터끼리 충돌하면 충돌하기 이전 위치로 이동한다.");
	block[126] = new Block("탄환 삭제", "게임 화면 내에 생성되어 있는 탄환을 모두 삭제한다.");
